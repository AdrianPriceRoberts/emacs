...
doom-themes makes =gnus-group-news-low-empty= inherit from
=gnus-group-news-low=, but Emacs 31's gnus defines =gnus-group-news-low= to
inherit from =gnus-group-news-low-empty=. Every time faces are recomputed
(loading gnus, or creating any new frame -- e.g. ediff's control frame) that
is an inheritance cycle and an error, which aborts whatever was running.
Replace the theme's own spec: a =user= override isn't enough, since
=face-spec-recalc= still applies the theme's spec on the way.
#+begin_src emacs-lisp
...
* Browser Infrastructure (tablist)
=*Lab Notebook*= and =*Bibliography*= (and any future "browse a list of
things" buffer) share the same shape: a =tabulated-list-mode= table with
sorting, filtering, a per-session row cache, and a keybinding hint.
Rather than copy that shape between modes, or invent a column-scoped
filter engine from scratch, this factors it into one minor mode,
=my/browser-mode=, built on [[https://github.com/politza/tablist][tablist]]
-- an existing, general "Ibuffer/Dired-style marking and filtering for
tabulated-list-mode" package, on NonGNU ELPA -- rather than reinventing
that part. tablist's filter engine natively understands compound
expressions (=Year == "2020" && Title =~ "distillation"=, with
=&&=/=||=/=!=/parens) stored as a stack you push onto -- so pressing ~/~
twice ANDs two separate column filters together -- and applies them by
*hiding* non-matching rows rather than rebuilding the list, which
composes cleanly with the row cache below. =my/browser-filter= is a thin
live-typing wrapper around tablist's own =tablist-push-filter=/
=tablist-apply-filter= -- no forking or patching of tablist itself, just
driving its public API from a =post-command-hook=, the same trick the
live citation/note pickers already use.

Deliberately reserved for later, not built now: =my/browser-point-entry-functions=,
an empty hook firing when point moves onto a different row, for future
per-row features (e.g. wrapping a long title, previewing an abstract) to
hang off of without needing to touch this section again.
...
Keys inside =*Lab Notebook*= (also shown in the echo area once idle, via
eldoc -- it won't clobber a message a command just printed, and appears
after 1.5s rather than immediately): ~n~/~p~ move the selection (built
into =tabulated-list-mode=, same as Dired), ~RET~ opens the entry, ~s~
prompts for a column to sort by (press again on the same column to flip
direction; clicking a column header does the same), ~/~ prompts for a
column to filter by (or "All columns") and pushes a live filter for it --
comma/semicolon-separate multiple keywords for an OR match, press ~/~
again to AND another column's filter on top (e.g. Date =2026= then Title
=catalyst= shows only 2026 entries mentioning it) -- ~c~ clears every
active filter, and ~g~ forgets the cached row list and re-scans Vulpea
(only needed after adding/editing notes without leaving this buffer --
opening it fresh always re-scans; an active filter stays applied across a
refresh). Also gets tablist's own marking commands for free (~m~ mark,
~u~ unmark, ~*~-prefixed mark-by-regexp/numeric, etc.) -- not wired to
anything yet, but there if useful later.
...
ask for explicit confirmation first. "Empty" means genuinely nothing worth
keeping: for a daily journal file (=~/org/daily/*.org=) that means the buffer
is still just the unfilled template (properties drawer, title/filetags/created,
the bare "* Notes" heading) with no notes actually typed in; for every other
file it's the original, narrower zero-byte check.
#+begin_src emacs-lisp
...
** Journal conflict guard
=global-auto-revert-mode= silently reverts any /unmodified/ buffer whose file
changes on disk -- including when Syncthing resolves a sync conflict by
writing a different machine's version over =~/org/daily/*.org=. That path
never calls =save-buffer= or =ask-user-about-supersession-threat=, so neither
guard above ever runs for it -- it's a different code path entirely (this is
what actually destroyed a day of journal notes: the buffer was clean, so
auto-revert silently mirrored in the empty version Syncthing wrote, with no
prompt at all).

This hooks the one thing =auto-revert-handler= always consults before
touching the buffer: =buffer-stale-function=. On daily journal buffers, a
template-only buffer still auto-reverts silently exactly as before (nothing
worth protecting), but a buffer with real content blocks the automatic
revert and prompts to keep the buffer, take the disk version, or open an
ediff session to merge the two by hand.

=my/diff-and-choose= is the shared "show the diff, then ask" step used here
and by the Syncthing conflict-file handling below. Its prompt always goes to
the minibuffer: these prompts fire from timers, where a preceding mouse click
would otherwise make Emacs pop up a GUI dialog instead.
#+begin_src emacs-lisp
...
Syncthing never merges: when two machines both changed a file, the copy with
the newest mtime keeps the real name and the other is renamed to
=NAME.sync-conflict-YYYYMMDD-HHMMSS-DEVICE.org=. The classic case here is
opening Emacs on a machine before Syncthing has pulled today's journal -- the
fresh template is newer, so it wins and the real notes land in the conflict
file, with no prompt anywhere.

So "conflict copy" usually means the /older/ version, and naming the choices
after Syncthing's roles ("keep original" / "take conflict") once led me to
overwrite a day's work with the old copy. The prompt is therefore built
around age and the diff symbols, never the roles:

- The =*Diff*= always runs OLDER -> NEWER, whichever file is which (by save
  time; unsaved typing in the original's buffer counts as newest). So =-=
  lines are only in the older version and =+= lines only in the newer one. A
  header on the diff says which file and device each side is, and when it
  was saved; the minibuffer repeats it.
- The keys are the diff symbols themselves:
  - ~+~ :: keep the newer version; every =-= line is deleted
  - ~-~ :: keep the older version; every =+= line is deleted
  - ~b~ both :: keep every line of both: shared lines appear once, and where
    they differ the original's lines come first. If both machines changed
    the /same/ line you'll see both versions next to each other to tidy up.
  - ~s~ skip :: leave both alone; not asked about again until Emacs restarts
- Nothing is written until a second =*Diff*= previews the original file NOW
  -> AFTER (=-= deleted from it, =+= added to it). Losing any line needs a
  typed =yes=; declining goes back to the choice.
- Before any resolution, including the automatic ones below, both versions
  are copied to =~/.emacs.d/sync-conflict-archive/TIMESTAMP-NAME/= (outside
  =~/org=, so Syncthing and org-roam ignore it, and git-ignored).
  ~M-x my/sync-conflict-undo-last~ puts the file and its conflict copy back
  exactly as they were, so it can be resolved again; repeating it goes
  further back.

Writes to the original go through its normal buffer, so the save guards and
backups above still apply (and ~C-/~ in that buffer undoes it), and "trash"
means the system trash, not a hard delete.

Each machine gives its copy of a new note its own random =:ID:=, so a merge
would otherwise leave two =:ID:= lines in one drawer -- and vulpea's indexer
takes the first while =org-entry-get= takes the last, so the database and
the open buffer disagree about which note this is. After any write, repeated
properties in a drawer are reduced to the first (the original's, since its
lines come first). If a note that disappears that way -- or by keeping only
the conflict copy's side, which swaps in its ID -- is linked from anywhere under
=~/org=, it offers to point those links at the ID that was kept.
~M-x my/fix-duplicate-properties~ does the same clean-up on the current
buffer, for files merged before this existed.

Cases where one side has nothing worth keeping are resolved without asking:
a conflict copy that is identical to the original or is just an unfilled
journal template is trashed, and an original that is just an unfilled
journal template (the startup race above) is replaced by the conflict copy.
For the original, the open buffer is what counts, so unsaved typing is never
treated as "just the template".

A startup scan and a file watch on =~/org/daily= offer to walk through all
conflict files whenever any exist; ~M-x my/resolve-sync-conflict~ handles one
on demand.
#+begin_src emacs-lisp
...
for "cite this" or "jump to my note on this paper," not for browsing. For
a persistent, sortable, filterable view of the whole library, this shares
=my/browser-mode= (see "Browser Infrastructure" near the top of this
file) with =*Lab Notebook*= -- only the columns, row-building, and
actions below are specific to citations.
#+begin_src emacs-lisp
...
Keys inside =*Bibliography*= (also shown in the echo area once idle, via
eldoc): ~RET~ opens (or creates) the paper's reference note, ~o~ opens its
library PDF, ~s~ prompts for a column to sort by (press again on the same
column to flip direction), ~/~ prompts for a column to filter by (or "All
columns") and pushes a live filter for it -- comma/semicolon-separate
multiple keywords for an OR match, press ~/~ again to AND another
column's filter on top (e.g. Year =2026= then Title =distillation,
extraction= shows only 2026 papers about either) -- ~c~ clears every
active filter, and ~g~ re-scans citar's library for =.bib= changes (an
active filter stays applied). Same keys as =*Lab Notebook*=, because both
share =my/browser-mode=.
