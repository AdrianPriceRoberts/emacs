...
Each conflict is handled the same way as the journal conflict guard above:
a =*Diff*= of the original (=-= lines) against the conflict copy (=+= lines),
then a minibuffer question:

- ~m~ merge both :: combine the two files, keeping every line from each:
  shared lines appear once, and where they differ the original's lines come
  first, then the conflict copy's. The result is saved to the original and
  the conflict copy is trashed. No windows open; if both machines changed the
  /same/ line you'll see both versions next to each other in the journal and
  can tidy up there.
- ~k~ keep original :: trash the conflict copy
- ~t~ take conflict :: replace the original's contents with the conflict copy, then trash it
...
backups above still apply, and "trash" means the system trash, not a hard
delete.
...
lines come first). If a note that disappears that way -- or through ~t~,
which swaps in the conflict copy's ID -- is linked from anywhere under
