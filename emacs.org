...
#+end_src
* Ivy Prescient
Out-of-order character-fuzzy matching (e.g. "smh" matches "Smith") plus
frecency-based ranking (recently/frequently picked candidates sort higher),
for every Ivy/Counsel-backed command at once: =M-x=, find-file,
=counsel-ibuffer=, swiper, =vulpea-find=, =vulpea-insert=, and citar's
pickers all get this for free since they all go through Ivy's
=completing-read=.
#+begin_src emacs-lisp
