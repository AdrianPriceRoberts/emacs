...
Keys inside =*Lab Notebook*=: ~n~/~p~ move the selection (built into
=tabulated-list-mode=, same as Dired), ~RET~ opens the entry, ~s~ prompts for a
column to sort by (press again on the same column to flip direction; clicking
a column header does the same), ~/~ filters to entries whose Page ID or Title
contains a substring (e.g. =APR= to show just that notebook), and ~c~ clears
the filter.
...
#+end_src
* Ivy Prescient
Out-of-order character-fuzzy matching (e.g. "smh" matches "Smith") for every
Ivy/Counsel-backed command: =M-x=, find-file, =counsel-ibuffer=, swiper,
=vulpea-find=, =vulpea-insert=, and citar's pickers.

Prescient's *ranking*, though, is frecency-based (recently/frequently
*picked* candidates sort first) with no concept of match quality -- ties
(which is everything, the first time you search for something) fall back to
shortest-candidate-first. That's actively wrong for a "find this specific
note/paper by typing part of its name" command: a long, exact-prefix match
loses to a short, barely-related one on pure length, burying the note you
actually want. It's the right behavior for something like =M-x= or
=ivy-switch-buffer=, where "the thing I pick most often" genuinely is what
you want back first -- which is why prescient's own author already excludes
=swiper= from frecency-sorting by default. The fix here is to extend that
same exclusion to every "search by content" command in this config, so they
still get prescient's better *filtering* but fall back to Ivy's own
(unsorted, filter-order) ranking instead of frecency:
#+begin_src emacs-lisp
...
Keys inside =*Bibliography*=: ~RET~ opens (or creates) the paper's
reference note, ~o~ opens its library PDF, ~s~ prompts for a column to sort
by (press again on the same column to flip direction), ~/~ filters to
entries whose Year/Author/Title/Tags contain a substring, and ~c~ clears
the filter -- the same keys as =*Lab Notebook*=, deliberately.
