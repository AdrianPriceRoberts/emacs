...
after 1.5s rather than immediately): ~n~/~p~ move
the selection (built into =tabulated-list-mode=, same as Dired), ~RET~
opens the entry, ~s~ prompts for a column to sort by (press again on the
same column to flip direction; clicking a column header does the same), ~/~
prompts for a column to filter by (or "All columns") and then filters
*live* in just that column -- the table updates after every keystroke, the
same way =vulpea-find= narrows as you type (e.g. pick "Date", type =2026= to
show only that year; =C-g= at either prompt cancels and restores the
previous filter), ~c~ clears the filter, and ~g~ forgets the cached row
list and re-scans Vulpea (only needed after adding/editing
...
eldoc): ~RET~ opens
(or creates) the paper's reference note, ~o~ opens its library PDF, ~s~
prompts for a column to sort by (press again on the same column to flip
direction), ~/~ prompts for a column to filter by (or "All columns") and
then filters *live* in just that column, updating after every keystroke,
the same way =citar-insert-citation= narrows as you type -- e.g. pick
"Year", type =2026= to show only that year's papers (=C-g= at either prompt
cancels and restores the previous filter) -- ~c~ clears the filter, and ~g~
re-scans citar's library (only needed after the =.bib= changes without
leaving this buffer) -- the same keys as =*Lab Notebook*=, deliberately.
